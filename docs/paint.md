...
| Save as | Store the image of the current window into a file. A dialog is opened to query for a file name. By default a raw format is used that stores the image row by row as a stream of pixels. The size of the stored image and other image formats can be selected in the Attributes menu. Be default the full image is stored, but you can restrict the area to the one currently visible in the window, or to specific coordinates. When the latter option is selected, a dialog opens upon saving which asks for entering the coordinates. The dialog is initialized with the last two coordinates used for drawing, or the two last mouse clicks when using option `Save coordinates` from the Tools menu. |
...
| Operation | Description |
|:----------|:------------|
| Save coordinates | This option replaces the regular drawing tools. Instead, a click into the image window will push the mouse coordinates of that click to a buffer, which tracks the last two coordinates. The only use for that is the `Coordinates` dialog. This dialog is opened either directly via the following menu command, or as part of saving to a file or printing when configured to save a specific region only (see description above). |
